...
namespace Phyah.Web.Razor
{
    public class RoslynCompileService : IRazorCompileService
    {
        ...
        private static List<MetadataReference> GetApplicationReferences()
        {
            ...
            var mscorlibFile = System.IO.Path.Combine(runtimePath, "mscorlib.dll");
            if (File.Exists(mscorlibFile))
                metadataReferences.Add(CreateMetadataFileReference(mscorlibFile));
            else
                metadataReferences.Add(CreateMetadataFileReference(System.IO.Path.Combine(runtimePath, "mscorlib.ni.dll")));
